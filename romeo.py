...
if str(sys.argv[1]) == "encrypt":
    ...
    if msg == "":
        ...
        encrypted_msg = ""
        for char in msg:
            encrypted_msg += str(encryptdict[char]) + ","

        encrypted_msg = encrypted_msg[:-1]
    with open('./encrypted_msg.txt', 'w', encoding='utf-8') as f:
        f.write(encrypted_msg)

elif str(sys.argv[1]) == "decrypt":
    with open('./encrypted_msg.txt', 'r', encoding='utf-8') as f:
    ...
    if encrypted_msg == "":
        ...
        for item in num_array:
            decrypted_msg += str(decryptdict[int(item)])
